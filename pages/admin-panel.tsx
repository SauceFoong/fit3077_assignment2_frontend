       ...
       <Layout auth = {auth}>
           ...
           <div>
            ...
            <div>
                ...
                {
                    booking.length != 0 ?
                        (Date.parse(booking["startTime"]) < Date.parse(Date())) && booking["status"] != "CANCELED" ? 
                            <span className="reminder">
                                <style>{css}</style>
                        ...
                        : 
                            <div>
                                <br />
                                <button className="w-100 btn btn-lg btn-primary" onClick={(e) => setChecked(!checked)}>
                                    Modify Booking
                                </button>
                                <br />
                                <div>
            ...
            <select className="w-100" name="previousRecord">
                <option value = {"None"}>---</option>
            {   
                'pastChanges' in booking["customer"]["additionalInfo"] ? 
                booking["customer"]["additionalInfo"]["pastChanges"].map((changes) => {
                    const data = {
