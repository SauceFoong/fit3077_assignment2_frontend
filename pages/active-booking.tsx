...
`;
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(data);
    console.log("This is the Active Booking Id", activeBooking[0]["id"]);

    await fetch(
      "http://localhost:8080/api/change_booking/" + activeBooking[0]["id"],
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: data,
  ...
  const cancelBooking = async (event) => {
    ...
    await fetch(
      "http://localhost:8080/api/cancel_booking/" + activeBooking[0]["id"],
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      }
  ...
  return (
    <Layout auth={auth}>
      ...
      {activeBooking.length != 0 ? (
        activeBooking[0]["status"] === "CANCELED" ? (
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="row">
                  ...
                  <select className="w-100" name="previousRecord">
                    <option value={"None"}>---</option>
                    {"pastChanges" in
                    activeBooking[0]["customer"]["additionalInfo"] ? (
                      activeBooking[0]["customer"]["additionalInfo"][
                        "pastChanges"
                      ].map((changes) => {
                        const data = {
